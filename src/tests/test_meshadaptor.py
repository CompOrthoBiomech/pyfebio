...
def test_hex_refine_adaptor(hex8_febmesh, tmp_path):
    my_model = feb.model.Model(mesh=hex8_febmesh, control=feb.model.Control(time_steps=3, step_size=1 / 3.0, time_stepper=None))
    ...
    my_model.boundary.add_bc(
        feb.boundary.BCPrescribedDisplacement(node_set="top", dof="z", value=feb.boundary.Value(lc=1, text=1.0))
    )
...
def test_mmg_remesh_adaptor(tet4_febmesh, tmp_path):
    my_model = feb.model.Model(mesh=tet4_febmesh, control=feb.model.Control(time_steps=4, step_size=0.25, time_stepper=None))
    for i, element in enumerate(my_model.mesh.elements):
        my_model.material.add_material(feb.material.NeoHookean(name=element.name, id=i + 1))
        my_model.mesh_domains.add_solid_domain(feb.meshdomains.SolidDomain(name=element.name, mat=element.name))
